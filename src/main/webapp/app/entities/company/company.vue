<template>
  <div>
      ...
      <div class="d-flex justify-content-end">
        ...
        <router-link :to="{ name: 'CompanyCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('gma2App.company.home.createLabel')"> Create a new Company </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && companies && companies.length === 0">
      <span v-text="$t('gma2App.company.home.notFound')">No companies found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="companies">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('gma2App.company.name')">Name</span></th>
            <th scope="row"><span v-text="$t('gma2App.company.tel')">Tel</span></th>
            <th scope="row"><span v-text="$t('gma2App.company.addr')">Addr</span></th>
            <th scope="row"><span v-text="$t('gma2App.company.email')">Email</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="gma2App.company.delete.question" data-cy="companyDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-company-heading" v-text="$t('gma2App.company.delete.question', { id: removeId })">
