      ...
      <table class="table table-striped" aria-describedby="stds">
        ...
        <tbody>
          <tr v-for="std in stds" :key="std.id" data-cy="entityTable">
            ...
            <td class="text-right">
              <div class="btn-group">
                <button class="btn btn-warning btn-sm details" v-on:click="prepareSendMail(std)" v-b-modal.sendMail>
              ...
              <div class="btn-group">
                <router-link :to="{ name: 'StdView', params: { stdId: std.id } }" custom v-slot="{ navigate }">
                  <button @click="navigate" class="btn btn-info btn-sm view" data-cy="entityViewButton">
