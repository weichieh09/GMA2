...
import { ICountryMark, CountryMark } from '@/shared/model/country-mark.model';
...
export default class CountryMarkUpdate extends Vue {
  @Inject('countryMarkService') private countryMarkService: () => CountryMarkService;
  @Inject('alertService') private alertService: () => AlertService;

  public countryMark: ICountryMark = new CountryMark();
    ...
    if (this.countryMark.id) {
      this.countryMarkService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('gma2App.countryMark.updated', { param: param.id });
      ...
      this.countryMarkService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('gma2App.countryMark.created', { param: param.id });
  ...
  public retrieveCountryMark(countryMarkId): void {
    this.countryMarkService()
      .find(countryMarkId)
      .then(res => {
