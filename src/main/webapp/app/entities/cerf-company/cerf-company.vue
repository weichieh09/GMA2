<template>
  <div>
    <h2 id="page-heading" data-cy="CerfCompanyHeading">
      <span v-text="$t('gma2App.cerfCompany.home.title')" id="cerf-company-heading">Cerf Companies</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('gma2App.cerfCompany.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CerfCompanyCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('gma2App.cerfCompany.home.createLabel')"> Create a new Cerf Company </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && cerfCompanies && cerfCompanies.length === 0">
      <span v-text="$t('gma2App.cerfCompany.home.notFound')">No cerfCompanies found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('relType')">
              <span v-text="$t('gma2App.cerfCompany.relType')">Rel Type</span>
            ...
            <th scope="row" v-on:click="changeOrder('cerf.id')">
              <span v-text="$t('gma2App.cerfCompany.cerf')">Cerf</span>
            ...
            <th scope="row" v-on:click="changeOrder('company.id')">
              <span v-text="$t('gma2App.cerfCompany.company')">Company</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="gma2App.cerfCompany.delete.question" data-cy="cerfCompanyDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-cerfCompany-heading" v-text="$t('gma2App.cerfCompany.delete.question', { id: removeId })">
          Are you sure you want to delete this Cerf Company?
        </p>
      </div>
