    ...
    <div class="col-8">
      <form name="editForm" role="form" novalidate v-on:submit.prevent="save()">
        ...
        <div>
          ...
          <div class="form-group">
            <label v-text="$t('gma2App.sticker.mark')" for="sticker-mark">Mark</label>
            <select
              class="form-control"
              id="sticker-marks"
              data-cy="mark"
              multiple
              name="mark"
              v-if="sticker.marks !== undefined"
              v-model="sticker.marks"
            >
              <option v-bind:value="getSelected(sticker.marks, markOption)" v-for="markOption in marks" :key="markOption.id">
                {{ markOption.id }}
              </option>
            </select>
          </div>
          <div class="form-group">
            <label class="form-control-label" v-text="$t('gma2App.sticker.prod')" for="sticker-prod">Prod</label>
            <select class="form-control" id="sticker-prod" data-cy="prod" name="prod" v-model="sticker.prod">
              <option v-bind:value="null"></option>
              <option
                v-bind:value="sticker.prod && prodOption.id === sticker.prod.id ? sticker.prod : prodOption"
