    ...
    <div class="col-8">
      <form name="editForm" role="form" novalidate v-on:submit.prevent="save()">
        ...
        <div>
          ...
          <div class="form-group">
            <label v-text="$t('gma2App.cerf.mark')" for="cerf-mark">Mark</label>
            <select
              class="form-control"
              id="cerf-marks"
              data-cy="mark"
              multiple
              name="mark"
              v-if="cerf.marks !== undefined"
              v-model="cerf.marks"
            >
              <option v-bind:value="getSelected(cerf.marks, markOption)" v-for="markOption in marks" :key="markOption.id">
                {{ markOption.id }}
              </option>
            </select>
          </div>
