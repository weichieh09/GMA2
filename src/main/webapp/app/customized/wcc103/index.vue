<template>
  <b-container fluid>
    <b-row>
      <h2 id="page-heading">
        <span v-text="$t('gma2App.wcc103.home.title')">wcc103</span>
      </h2> </b-row
    ><br />
            ...
            <b-col cols="12" class="d-flex justify-content-end">
              <b-button-group>
                <b-button @click="onSubmit" variant="primary"
                  ><b-icon icon="search"></b-icon> <span v-text="$t('gma2App.wcc103.home.submitLabel')">search</span></b-button
                >
                <b-button @click="onReset" variant="outline-secondary"
                  ><b-icon icon="eraser"></b-icon> <span v-text="$t('gma2App.wcc103.home.resetLabel')">eraser</span></b-button
                >
              </b-button-group>
            </b-col>
          </b-row>
    ...
    <b-row>
      <b-col cols="12">
        <div class="alert alert-warning" v-if="isFetch && csvList && csvList.length === 0">
          <span v-text="$t('gma2App.wcc103.home.notFound')">No data found</span>
        </div>
        <div class="alert alert-warning" v-if="isNull">
          <span v-text="$t('gma2App.wcc103.home.cantNull')">Can't Null</span>
        </div>
      </b-col>
      <b-col cols="8">
        <div v-if="csvList && csvList.length > 0">
          <span>
            <h3>生產廠Q: {{ searchName }}</h3>
          </span>
        </div>
        <div class="table-responsive" v-if="csvList && csvList.length > 0">
          <table class="table table-striped" aria-describedby="csvList">
            <thead>
              <tr>
                <th scope="row">
                  <span v-text="$t('gma2App.wcc103.table1.areaCd')">Area Cd</span>
                </th>
                <th scope="row">
                  <span v-text="$t('gma2App.wcc103.table1.cerfNo')">Cerf No</span>
                </th>
                <th scope="row">
                  <span v-text="$t('gma2App.wcc103.table1.cerfVer')">Cerf Ver</span>
                </th>
                <th scope="row">
                  <span v-text="$t('gma2App.wcc103.table1.cerfFee')">Cerf Fee</span>
                </th>
                <th scope="row">
                  <span v-text="$t('gma2App.wcc103.table1.stsCd')">Sts Cd</span>
                </th>
                <th scope="row">
                  <span v-text="$t('gma2App.wcc103.table1.applId')">Appl Id</span>
                </th>
                <th scope="row"></th>
              </tr>
            </thead>
