...
public class CerfRepositoryWithBagRelationshipsImpl implements CerfRepositoryWithBagRelationships {
    ...
    public Optional<Cerf> fetchBagRelationships(Optional<Cerf> cerf) {
        return cerf.map(this::fetchProds).map(this::fetchStds).map(this::fetchMarks);
    }
    ...
    public List<Cerf> fetchBagRelationships(List<Cerf> cerfs) {
        return Optional.of(cerfs).map(this::fetchProds).map(this::fetchStds).map(this::fetchMarks).orElse(Collections.emptyList());
    }
    ...
    Cerf fetchMarks(Cerf result) {
        return entityManager
            .createQuery("select cerf from Cerf cerf left join fetch cerf.marks where cerf is :cerf", Cerf.class)
            .setParameter("cerf", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Cerf> fetchMarks(List<Cerf> cerfs) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, cerfs.size()).forEach(index -> order.put(cerfs.get(index).getId(), index));
        List<Cerf> result = entityManager
            .createQuery("select distinct cerf from Cerf cerf left join fetch cerf.marks where cerf in :cerfs", Cerf.class)
            .setParameter("cerfs", cerfs)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getResultList();
        Collections.sort(result, (o1, o2) -> Integer.compare(order.get(o1.getId()), order.get(o2.getId())));
