...
public class ProdRepositoryWithBagRelationshipsImpl implements ProdRepositoryWithBagRelationships {
    ...
    public Optional<Prod> fetchBagRelationships(Optional<Prod> prod) {
        return prod.map(this::fetchCountries).map(this::fetchStds).map(this::fetchMarks);
    }
    ...
    public List<Prod> fetchBagRelationships(List<Prod> prods) {
        return Optional.of(prods).map(this::fetchCountries).map(this::fetchStds).map(this::fetchMarks).orElse(Collections.emptyList());
    }
    ...
    Prod fetchMarks(Prod result) {
        return entityManager
            .createQuery("select prod from Prod prod left join fetch prod.marks where prod is :prod", Prod.class)
            .setParameter("prod", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Prod> fetchMarks(List<Prod> prods) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, prods.size()).forEach(index -> order.put(prods.get(index).getId(), index));
        List<Prod> result = entityManager
            .createQuery("select distinct prod from Prod prod left join fetch prod.marks where prod in :prods", Prod.class)
            .setParameter("prods", prods)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getResultList();
        Collections.sort(result, (o1, o2) -> Integer.compare(order.get(o1.getId()), order.get(o2.getId())));
